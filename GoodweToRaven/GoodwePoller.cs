...
namespace GoodweToRaven
{
    public class GoodwePoller
    {
        ...
        public async IAsyncEnumerable<Inverter> DiscoverInvertersAsync(string host = "192.168.2.255")
        {
        ...
        private async Task SendHello(UdpClient client, string host)
        {
            var payload = Encoding.ASCII.GetBytes("WIFIKIT-214028-READ");
            await client.SendAsync(payload, payload.Length, host, 48899);
        }
